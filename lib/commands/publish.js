...
function publish(build, options) {
  ...
  return transport.beforeUpload(build)
    .then(() => {
      return publishAssets(build, transport, options);
    })
    .then((assets) => {
 ...
 */
function publishAssets(build, transport, options) {
    ...
    promise = promise
      .then(() => {
        if (uploaded[filePath] === undefined) {
          return transport.uploadFile(filePath, build, options);
        } else {
...
function publishOsxReleaseFile(build, options, assets, transport) {
  ...
  return transport.uploadFile(releaseFilePath, build, options)
    .then((releaseUrl) => {
