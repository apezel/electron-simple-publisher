...
class GithubApi {
   ...
   */
  releaseFile(filePath, tag, options, onProgress) {
    ...
    return this.request('GET /repos/:owner/:repo/releases/tags/:tag', { _tag:
      ...
      .then((result) => {
        let description;
        if (options.transport.changelog) {
          let changelogPath = options.transport.changelog.replace(/\{version\}/, "v"+options.version);
          description = fs.readFileSync(path.join(process.cwd(), changelogPath), "utf8");
        }
